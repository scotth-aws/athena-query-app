...
const Content = (user) => {
  ...
  const [geneNameValue, setGeneNameValue] = React.useState("");
  ...
  const geneNameChange = (detail) => {

    
    setGeneNameValue(detail);
  ...
  const submitFormHandler = (event, selectValue) => {
    ...
    console.log('Gene Name '+JSON.stringify(geneNameValue));
    setDisabled(true);
    ...
    clinvarPredicate = "a.genename = "+"'"+geneNameValue+"'";
    console.log('clinvarPredicate '+clinvarPredicate);
    description = 'A report on all variants for specific Gene Name';
    name = 'List of all variants where Gene Name is ' + geneNameValue;
          ...
          >
            ...
            <SpaceBetween direction="vertical" size="s">
              <FormField
                ...
                errorText={!selectValue && "Input Gene Name"}
              >
                <TextContent></TextContent>

                <Input      
                onChange={({ detail }) => geneNameChange(detail.value)}      
                value={geneNameValue}    
...
export default withAuthenticator(QueryFilterGeneName);;
